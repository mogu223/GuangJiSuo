...
class rbklib:
    ...
    def lock(self,name="rbklib"):
        so = self.so_19207
        data = {"nick_name":name}
        msg = packMasg(1, 4005, data)
        so.send(msg) 
        result= self.getResponse(so)
        return result
    ...
    def request(self, msgType: int, reqId: int = 1, msg = None,sock: socket.socket = None) :
        """
        发送请求
