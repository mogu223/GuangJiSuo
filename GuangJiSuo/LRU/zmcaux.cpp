...
{
	return  ZAux_Direct_SetUserVar(handle, (char*)"P_BusCmd",2);
}
...
int32  ZAux_BusCmd_GetInitStatus(ZMC_HANDLE handle,int *piValue)
{
	...
	iresult = ZAux_Direct_GetUserVar(handle, (char*)"Bus_Scan_Status",&scan_flag);
	if(iresult != ERR_OK)
	{
		return iresult;
	}
	iresult = ZAux_Direct_GetUserVar(handle, (char*)"Bus_Start_Status",&start_flag);
	if(iresult != ERR_OK)
	{
		return iresult;
	}
...
Return:         //???
*************************************************************/
int32 __stdcall ZAux_Direct_SetUserArray_Char(ZMC_HANDLE handle,char * arrayname ,int arraystart, int numes, char *pfValue)
{
    int i;
    int32 iresult;
    ...
    {
        //
        sprintf(cmdbuff, "%s(%d) =  %d ",arrayname ,(i + arraystart) , pfValue[i]);

        iresult = ZAux_DirectCommand(handle, cmdbuff, cmdbuffAck, 2048);
        //qDebug()<<cmdbuff<<numes<<iresult;
        if(ERR_OK != iresult)
        {
            return iresult;
        }
