...
enum string_types 
{
	...
	STRING_ZVOBJ = 9, // zv 类型

    STRING_UNKOWN = 10, 

	// 增加结构的支持, UNION也是用这个
	STRING_USERSTRUCT = 11, 
	STRING_STRUCTVARIABLE = 12, // 
	STRING_STRUCTARRAY = 13, 
};
...
enum AXIS_FEATURE_BIT
{
	...
	AXIS_FEATURE_OP = 15, //  输出口共用的轴
	// 增加更多的属性
	AXIS_FEATURE_ORIGIN_STEPPER = 16,  // 原生脉冲
	AXIS_FEATURE_ORIGIN_ENCODER = 17,  // 原生编码器, 强制指定时优先使用

	//
	AXIS_FEATURE_2_STEPPER = 18,  // 第二FPGA
	AXIS_FEATURE_2_ENCODER = 19,  // 第二FPGA, 强制指定的时候, 优先使用第二FPGA.

	//
	AXIS_FEATURE_SCAN = 20, // 振镜， 物理轴号 4,6,偶数是X， 奇数是Y   
