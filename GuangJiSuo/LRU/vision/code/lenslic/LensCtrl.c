...
int UserAreaWrite(char* userName) {
	char sendData[USERAREA_LENGTH + SEGMENTOFFSET_LENGTH + 1];	// 32+2+1=35
	sendData[0] = USER_AREA >> 8;
	sendData[1] = USER_AREA;
	strcpy_s(sendData + SEGMENTOFFSET_LENGTH, USERAREA_LENGTH + 1, userName);
...
int ZoomMove(uint16_t addrData) {
	uint16_t moveValue = abs(addrData - zoomCurrentAddr);
	int waitTime = WaitCalc(moveValue, zoomSpeedPPS);
